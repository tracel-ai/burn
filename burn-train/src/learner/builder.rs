  ...
  S: LrScheduler,
{
    ...
    S::Record: 'static,
  {
    ...
    let renderer = self
      .renderer
      .unwrap_or_else(|| Box::new(default_renderer(self.interrupter.clone(), self.checkpoint)));
    let directory = &self.directory;

    if self.num_loggers == 0 {
      self
        .event_store
        .register_logger_train(FileMetricLogger::new(format!("{directory}/train").as_str()));
      self
        .event_store
        .register_logger_valid(FileMetricLogger::new(format!("{directory}/valid").as_str()));
    }
