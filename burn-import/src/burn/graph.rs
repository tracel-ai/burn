    ...
    match record_type {
      RecordType::PrettyJson => {
        PrettyJsonFileRecorder::<PS>::new()
          .save_item(
            BurnRecord::new::<PrettyJsonFileRecorder<PS>>(StructMap(BurnGraphState::new(
              &self.nodes,
            ))),
            out_file.clone(),
          )
      ...
      RecordType::NamedMpkGz => {
        NamedMpkGzFileRecorder::<PS>::new()
          .save_item(
            BurnRecord::new::<NamedMpkGzFileRecorder<PS>>(StructMap(BurnGraphState::new(
              &self.nodes,
            ))),
            out_file.clone(),
          )
      ...
      RecordType::Bincode => {
        BinFileRecorder::<PS>::new()
          .save_item(
            BurnRecord::new::<BinFileRecorder<PS>>(StructTuple(BurnGraphState::new(&self.nodes))),
            out_file.clone(),
          )
          .unwrap();
