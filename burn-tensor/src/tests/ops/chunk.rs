...
mod tests {
    ...
    fn test_chunk_evenly_divisible() {
        let tensors: Vec<Tensor<TestBackend, 1, Int>> = Tensor::arange_default(0..12).chunk(6, 0);
    ...
    fn test_chunk_not_evenly_divisible() {
        let tensors: Vec<Tensor<TestBackend, 1, Int>> = Tensor::arange_default(0..11).chunk(6, 0);
        assert_eq!(tensors.len(), 6);
    ...
    fn test_chunk_not_evenly_divisible_remains_several() {
        let tensors: Vec<Tensor<TestBackend, 1, Int>> = Tensor::arange_default(0..100).chunk(8, 0);
    ...
    fn test_chunk_not_divisible() {
        let tensors: Vec<Tensor<TestBackend, 1, Int>> = Tensor::arange_default(0..6).chunk(7, 0);
        assert_eq!(tensors.len(), 6);
    ...
    fn test_chunk_multi_dimension() {
        let tensors: Vec<Tensor<TestBackend, 2, Int>> =
            Tensor::from_data_default(Data::from([[0, 1, 2, 3]])).chunk(2, 1);
    ...
    #[should_panic]
    fn test_invalid_dim() {
        let tensors: Vec<Tensor<TestBackend, 1, Int>> = Tensor::arange_default(0..12).chunk(6, 1);
    }
}
