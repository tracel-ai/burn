...
impl TensorCheck {
  ...
  pub(crate) fn into_scalar<const D: usize>(shape: &Shape<D>) -> Self {
    ...
    if shape.num_elements() != 1 {
      check = check.register(
        "Into Scalar",
        TensorError::new("Only tensors with 1 element can be converted into scalar.").details(
          format!("Current tensor has {} elements", shape.num_elements()),
        ),
      );
    }
  ...
  pub(crate) fn reshape_args_i32<const D: usize>(target: &[i32; D]) -> Self {
    ...
    if target.iter().any(|&dim| dim < -1) {
      check = check.register(
        "Reshape",
        TensorError::new("The given shape cannot contain negative dimensions (other than -1).")
  ...
  pub(crate) fn swap_dims<const D: usize>(dim1: usize, dim2: usize) -> Self {
    ...
    if dim1 > D || dim2 > D {
      check = check.register(
        "Swap Dims",
        TensorError::new("The swap dimensions must be smaller than the tensor dimension").details(
          format!("Swap dims ({dim1}, {dim2}) on tensor with ({D}) dimensions."),
        ),
      );
    }
  ...
  pub(crate) fn matmul<B: Backend, const D: usize>(lhs: &Tensor<B, D>, rhs: &Tensor<B, D>) -> Self {
    let mut check = Self::Ok;
  ...
  pub(crate) fn cat<B: Backend, const D: usize, K: BasicOps<B>>(
    ...
    if dim >= D {
      check = check.register(
        "Cat",
        TensorError::new("Can't concatenate tensors on a dim that exceeds the tensors dimension")
          .details(format!(
  ...
  pub(crate) fn scatter<const D: usize>(
    ...
    let ops = "Scatter";
    let mut check = Self::check_gather_scatter_indices(Self::Ok, ops, dim, shape, shape_indices);

    if shape_indices != shape_value {
      check = check.register(
        ops,
        TensorError::new(
          "Indices tensor shape should be the same as the value tensor shape.".to_string(),
        )
        .details(format!(
          "The shape differs: {:?} != {:?}",
    ...
    for i in 0..D {
      ...
      if tensor_dim_i != indices_dim_i {
        check = check.register(
          ops,
          TensorError::new(
            "The tensor shape should be the same as the index tensor shape.".to_string(),
          )
          .details(format!(
  ...
  pub(crate) fn binary_ops_ew_shape<const D: usize>(
    ...
    for i in 0..D {
      ...
      if d_lhs != d_rhs {
        ...
        check = check.register(
          ops,
          TensorError::new("The provided tensors have incompatible shapes.").details(format!(
  ...
  fn binary_ops_device<Device: PartialEq + core::fmt::Debug>(
    ...
    match lhs != rhs {
      true => self.register(
        ops,
        TensorError::new("The provided tensors are not on the same device.").details(format!(
          "Lhs tensor device {lhs:?}, Rhs tensor device {rhs:?}.",
        )),
      ),
      false => self,
    }
