...
pub(crate) fn conv_transpose2d_backward<B: Backend>(
  ...
  let weight_grad = match options.groups == 1 {
    true => {
      conv_transpose2d_weight_grad_no_groups::<B>(x, output_grad.clone(), weight_shape, options)
    }
...
pub(crate) fn conv_transpose1d_backward<B: Backend>(
  ...
  let weight_grad = match options.groups == 1 {
    true => {
      conv_transpose1d_weight_grad_no_groups::<B>(x, output_grad.clone(), weight_shape, options)
    }
...
fn calculate_padding_out(
  ...
  let out = 1
    + libm::ceil((size_in + 2 * padding - dilation * (kernel_size - 1) - 1) as f64 / stride as f64)
      as usize;
