  ...
  E: rand::distributions::uniform::SampleUniform,
{
  ...
  pub fn sampler<R: RngCore>(self, rng: &'_ mut R) -> DistributionSampler<'_, E, R> {
    let kind = match self {
      Distribution::Default => DistributionSamplerKind::Standard(rand::distributions::Standard {}),
      Distribution::Uniform(low, high) => {
        DistributionSamplerKind::Uniform(rand::distributions::Uniform::new(low, high))
      }
      Distribution::Bernoulli(prob) => {
        DistributionSamplerKind::Bernoulli(rand::distributions::Bernoulli::new(prob).unwrap())
      }
...
impl<E: core::fmt::Debug + Copy, const A: usize, const B: usize, const C: usize, const D: usize>
...
mod tests {
  ...
  fn should_have_right_num_elements() {
    ...
    let num_elements = shape.num_elements();
    let data = Data::<f32, 3>::random(shape, Distribution::Default, &mut StdRng::from_entropy());
