...
use crate::components_v2::{OutputTrainV2, OutputValidV2, TrainLoaderV2, ValidLoaderV2};
use crate::learner::EarlyStoppingStrategy;
use crate::learner::base::Interrupter;
use crate::learner::base_v2::LearnerV2;
use crate::learner::paradigms::{SupervisedLearningStrategy, TrainingStrategy};
...
use crate::multi_v2::MultiDeviceLearningStrategyV2;
use crate::renderer::{MetricsRenderer, default_renderer};
use crate::single_v2::SingleDevicetrainingStrategy;
use crate::{
    ApplicationLoggerInstaller, EarlyStoppingStrategyRef, FileApplicationLoggerInstaller,
    LearnerSummaryConfig, LearningComponents, LearningDataMarkerV2, LearningParadigm,
    ParadigmComponentMarker, ParadigmComponents, SupervisedComponentsMarker,
    SupervisedLearningComponents, TrainBackendV2, TrainStep, TrainingCheckpointer, TrainingResult,
    ValidStep,
};
...
impl<LC, TI, TO, VI, VO>
    SupervisedTraining<
        SupervisedComponentsMarker<
            ParadigmComponentMarker<
    ...
    VO: ItemLazy + 'static,
{
    ...
    pub fn new(
        ...
        dataloader_train: TrainLoaderV2<LC, LearningDataMarkerV2<TI, VI, TO, VO>>,
        dataloader_valid: ValidLoaderV2<LC, LearningDataMarkerV2<TI, VI, TO, VO>>,
    ) -> Self {
...
pub struct SupervisedTraining<SC: SupervisedLearningComponents> {
    ...
    checkpointers: Option<(
        AsyncCheckpointer<
            <<SC::LC as LearningComponents>::Model as Module<TrainBackendV2<SC::LC>>>::Record,
            TrainBackendV2<SC::LC>,
        >,
        AsyncCheckpointer<
            <<SC::LC as LearningComponents>::Optimizer as Optimizer<
                SC::Model,
                TrainBackendV2<SC::LC>,
            >>::Record,
            TrainBackendV2<SC::LC>,
        >,
        AsyncCheckpointer<
            <<SC::LC as LearningComponents>::LrScheduler as LrScheduler>::Record<
                TrainBackendV2<SC::LC>,
            >,
            TrainBackendV2<SC::LC>,
        >,
...
impl<SC: SupervisedLearningComponents> SupervisedTraining<SC> {
    ...
    where
        FR: FileRecorder<<SC::LC as LearningComponents>::Backend> + 'static,
        FR: FileRecorder<<<SC::LC as LearningComponents>::Backend as AutodiffBackend>::InnerBackend>
            + 'static,
...
pub struct TrainingComponents<SC: SupervisedLearningComponents> {
...
impl<SC: SupervisedLearningComponents + Send + 'static> LearningParadigm<SC::LC>
    for SupervisedTraining<SC>
{
    fn run(mut self, learner: LearnerV2<SC::LC>) -> TrainingResult<SC::InnerModel> {
        ...
        match self.training_strategy {
            ...
            TrainingStrategy::DistributedDataParallel { devices, config } => {
                use crate::ddp_v2::DdpTrainingStrategy;
...
pub trait MetricRegistrationV2<SC: SupervisedLearningComponents>: Sized {
    /// Register the metrics.
    fn register(self, builder: SupervisedTraining<SC>) -> SupervisedTraining<SC>;
}

/// Trait to fake variadic generics.
pub trait TextMetricRegistrationV2<SC: SupervisedLearningComponents>: Sized {
    /// Register the metrics.
    fn register(self, builder: SupervisedTraining<SC>) -> SupervisedTraining<SC>;
}
...
macro_rules! gen_tuple {
    ($($M:ident),*) => {
        impl<$($M,)* SC: SupervisedLearningComponents> TextMetricRegistrationV2<SC> for ($($M,)*)
        where
        ...
        impl<$($M,)* SC: SupervisedLearningComponents> MetricRegistrationV2<SC> for ($($M,)*)
        where
            $(<OutputTrainV2<SC::LD> as ItemLazy>::ItemSync: Adaptor<$M::Input>,)*
            $(<OutputValidV2<SC::LD> as ItemLazy>::ItemSync: Adaptor<$M::Input>,)*
