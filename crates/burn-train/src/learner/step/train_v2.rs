use crate::{
    InputTrainV2, OutputTrainV2, SupervisedLearningComponents, TrainBackendV2, TrainOutput, TrainStep,
};
...
pub struct MultiDevicesTrainStepV2<SC: SupervisedLearningComponents> {
    workers: Vec<Worker<SC>>,
    receiver: Receiver<MultiTrainOutput<OutputTrainV2<SC::LD>>>,
}
...
struct Worker<SC: SupervisedLearningComponents> {
    sender_input: Sender<Message<SC::Model, InputTrainV2<SC::LD>>>,
    device: Device<TrainBackendV2<SC::LC>>,
}

impl<SC: SupervisedLearningComponents> Worker<SC> {
    fn register(&self, item: InputTrainV2<SC::LD>, model: &SC::Model) {
        let message = Message {
            item,
            model: model.clone(),
        };
    ...
    fn start(
        &self,
        sender_output: Sender<MultiTrainOutput<OutputTrainV2<SC::LD>>>,
        receiver_input: Receiver<Message<SC::Model, InputTrainV2<SC::LD>>>,
    ) {
...
impl<SC: SupervisedLearningComponents> MultiDevicesTrainStepV2<SC> {
    ...
    pub fn new(devices: &[Device<TrainBackendV2<SC::LC>>]) -> Self {
    ...
    pub fn step<'a>(
        &self,
        dataloaders: &mut [Box<dyn DataLoaderIterator<InputTrainV2<SC::LD>> + 'a>],
        model: &SC::Model,
    ) -> (Vec<MultiTrainOutput<OutputTrainV2<SC::LD>>>, Progress) {
        let mut num_send = 0;
