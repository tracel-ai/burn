...
impl ClipVisualEncoderConfig {
    ...
    pub fn init<B: Backend>(&self, device: &B::Device) -> ClipVisualEncoder<B> {
        assert_eq!(
...
#[derive(Module, Debug)]
pub struct ClipVisualEncoder<B: Backend> {
...
impl<B: Backend> ClipVisualEncoder<B> {
...
impl TransformerBlockConfig {
    pub(crate) fn init<B: Backend>(&self, device: &B::Device) -> TransformerBlock<B> {
        TransformerBlock {
...
#[derive(Module, Debug)]
pub(crate) struct TransformerBlock<B: Backend> {
    pub(crate) ln_1: LayerNorm<B>,
    pub(crate) attn: ClipQkvAttention<B>,
    pub(crate) ln_2: LayerNorm<B>,
    pub(crate) mlp: TransformerMlp<B>,
}
...
impl TransformerMlpConfig {
    pub(crate) fn init<B: Backend>(&self, device: &B::Device) -> TransformerMlp<B> {
        TransformerMlp {
...
#[derive(Module, Debug)]
pub(crate) struct TransformerMlp<B: Backend> {
    pub(crate) c_fc: Linear<B>,
    pub(crate) activation: QuickGelu,
    pub(crate) c_proj: Linear<B>,
}

impl<B: Backend> TransformerMlp<B> {
    pub(crate) fn forward(&self, x: Tensor<B, 3>) -> Tensor<B, 3> {
