use crate::{LearningModel, metric::ItemLazy};
use burn_core::{
...
pub trait LearningComponents: Clone {
...
impl<B, LR, M, O> LearningComponents for LearnerComponentsMarkerV2<B, LR, M, O>
where
    B: AutodiffBackend,
...
pub type TrainBackendV2<LC> = <LC as LearningComponents>::Backend;
/// The validation backend.
pub type ValidBackendV2<LC> =
    <<LC as LearningComponents>::Backend as AutodiffBackend>::InnerBackend;
/// The training model.
pub(crate) type TrainModel<LC> = <LC as LearningComponents>::Model;
