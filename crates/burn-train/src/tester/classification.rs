...
#[derive(new)]
pub struct TestClassificationOutput<B: Backend> {
    /// The loss.
    pub loss: Tensor<B, 1>,
...
impl<B: Backend> Adaptor<AccuracyInput<B>> for TestClassificationOutput<B> {
    fn adapt(&self) -> AccuracyInput<B> {
        AccuracyInput::new(self.output.clone(), self.targets.clone())
    }
}

impl<B: Backend> Adaptor<LossInput<B>> for TestClassificationOutput<B> {
