...
use crate::compute::{
    dawn_native_bindings::*, webgpu_api::*, ComputeTask, WgpuServer, WgpuStorage,
};
    ...
    for DawnCommandEncoder
{
    fn dispatch_compute_pass(
        ...
        tasks: &[ComputeTask<DawnBindGroup, DawnComputePipeline>],
    ) {
        ...
        for task in tasks.iter() {
            unsafe {
                wgpuComputePassEncoderSetPipeline(pass, task.pipeline.pipeline.into());
                wgpuComputePassEncoderSetBindGroup(
                    pass,
                    0,
                    task.bind_group.bind_group.into(),
                    0,
                    (&[]).as_ptr(),
                );
