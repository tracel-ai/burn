...
pub(crate) fn launch_cmp<const D: usize, R: JitRuntime, E: JitElement, O: ComparisonOp<E>>(
    lhs: JitTensor<R, E, D>,
    rhs: JitTensor<R, E, D>,
) -> JitTensor<R, u32, D> {
    ...
    unsafe {
        if same_tensor_type && lhs.can_mut_broadcast(&rhs) {
            kernel_cmp::launch_unchecked::<E, O, R>(
                &client,
                cube_count,
                cube_dim,
...
>(
    ...
    unsafe {
        if same_tensor_type && tensor.can_mut() {
            kernel_scalar_cmp::launch_unchecked::<E, O, R>(
                &client,
                cube_count,
                cube_dim,
