...
use super::base::AccumulateDimNaive;

pub(crate) struct NaiveAccumulateDimComputeShader<E: JitElement, RD: AccumulateDimNaive<E>> {
    tensor: Variable,
    dim: usize,
    output: Variable,
    _accumulate_dim: PhantomData<RD>,
    _elem: PhantomData<E>,
}

#[derive(new)]
pub(crate) struct NaiveAccumulateDimEagerKernel<
    RD: AccumulateDimNaive<EI>,
    R: JitRuntime,
    EI: JitElement,
    EO: JitElement,
> {
    dim: usize,
    accumulate_dim: PhantomData<RD>,
    _runtime: PhantomData<R>,
    _elem_in: PhantomData<EI>,
    _elem_out: PhantomData<EO>,
}

impl<RD: AccumulateDimNaive<EI>, R: JitRuntime, EI: JitElement, EO: JitElement> Kernel
for NaiveReduceDimEagerKernel<RD, R, EI, EO>
{
    fn define(&self) -> KernelDefinition {
        ...
        NaiveAccumulateDimComputeShader {
            tensor,
            dim: self.dim,
            output,
            _accumulate_dim: PhantomData::<RD>,
...
impl<E: JitElement, RD: AccumulateDimNaive<E>> NaiveAccumulateDimComputeShader<E, RD> {
    pub(crate) fn expand(self, scope: &mut Scope) {
        ...
        cpa!(
            ...
            range(0u32, Variable::Rank).for_each(|i, scope| {
                ...
                let is_dim_accumulate = scope.create_local(Elem::Bool);
                cpa!(scope, is_dim_accumulate = i == dim);

                cpa!(scope, if(is_dim_accumulate).then(|scope|{
...
/// Executes the naive kernel for accumulate dim
pub fn accumulate_dim_naive<
    RD: AccumulateDimNaive<EI>,
    R: JitRuntime,
    EI: JitElement,
    EO: JitElement,
...
>(
    ...
    let kernel = NaiveAccumulateDimEagerKernel::<RD, R, EI, EO>::new(dim);
