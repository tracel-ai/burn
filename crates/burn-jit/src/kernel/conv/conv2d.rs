...
fn kernel<F: Float>(
    ...
    let in_channels = weight.shape(1u32);

    let kernel_size_0 = Comptime::unwrap_or_else(kernel_size_0_unroll, || weight.shape(2u32));
    let unroll_0 = Comptime::is_some(kernel_size_0_unroll);
    let kernel_size_1 = Comptime::unwrap_or_else(kernel_size_1_unroll, || weight.shape(3u32));
    let unroll_1 = Comptime::is_some(kernel_size_1_unroll);

    let b = AbsoluteIndex::get() / output.stride(0u32) % output.shape(0u32);
    let oc = AbsoluteIndex::get() / output.stride(1u32) % output.shape(1u32);
    let oh = AbsoluteIndex::get() / output.stride(2u32) % output.shape(2u32);
    let ow = AbsoluteIndex::get() / output.stride(3u32) % output.shape(3u32);

    let g = (weight.shape(0u32) + oc) % groups;
    ...
    let weight_stride_1 = weight.stride(1u32);
    let weight_stride_2 = weight.stride(2u32);
    let weight_stride_3 = weight.stride(3u32);

    let input_stride_1 = input.stride(1u32);
    let input_stride_2 = input.stride(2u32);
    let input_stride_3 = input.stride(3u32);
    let input_shape_2 = input.shape(2u32);
    let input_shape_3 = input.shape(3u32);
    ...
    let index_input_0 = b * input.stride(0u32);
    let index_weight_0 = oc * weight.stride(0u32);
