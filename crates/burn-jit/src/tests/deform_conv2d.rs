...
mod tests {
    ...
    impl DeformConv2dTestCase {
        fn assert_output(self, y: Tensor<TestBackend, 4>) {
            ...
            println!("Input: {:?}", x.to_data().to_vec::<f32>());
            println!("Offsets: {:?}", offset.to_data().to_vec::<f32>());
            println!("Weights: {:?}", weight.to_data().to_vec::<f32>());
            println!("Mask shape: {:?}", shape_mask);
            println!("Mask: {:?}", mask.to_data().to_vec::<f32>());
            println!("Bias: {:?}", bias.to_data().to_vec::<f32>());
