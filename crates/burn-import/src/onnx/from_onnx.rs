...
impl OnnxGraphIO {
    ...
    pub fn init_out(&mut self, proto_str: &str) {
        match self.old_io_names.get(proto_str) {
            ...
            Some(IOEntry::Node(i)) => {
                //TODO: this should panic if we rework node processing to allow nodes to be skipped
                //but currently we will initialize the same node twice when condensing nodes in coalesce
                log::warn!(
...
fn rename_io(node: &mut Node, graph_io: &mut OnnxGraphIO) {
    ...
    for node_input in node.inputs.iter_mut() {
        if let Some(input_name) = graph_io.get_new_name(node_input) {
            graph_io.set_passed(node_input);
