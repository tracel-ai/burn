...
impl OnnxGraphIO {
    ...
    pub fn init_out(&mut self, proto_str: &str) {
        match self.old_io_names.get(proto_str) {
            ...
            //aleady handled on graph_io creation
            Some(IOEntry::Out(i)) => {
                self.inputs[*i].passed = true;
            }
            Some(IOEntry::In(_)) => {
    ...
    pub(crate) fn copy_type(&mut self, in_name: &str, out_name: &str) {
        let ty = match self.old_io_names.get(in_name) {
            ...
            None => panic!("No entry {} to copy from", in_name),
        };

        match self.old_io_names.get(out_name) {
            Some(IOEntry::In(i)) => {
                self.inputs[*i].ty = ty;
            }
            Some(IOEntry::Out(i)) => {
            ...
            None => panic!("No entry for {} to copy to", out_name),
        }
    }

    /// Copy value and type from one IO entry to another
    pub fn copy_value_type(&mut self, in_name: &str, out_name: &str) {
        let value = self.get_value(in_name).cloned();
        let ty = self.get_type(in_name).clone();
    ...
    //         match self.old_io_names.get(node_output) {
    //             Some(IOEntry::In(_)) => {
...
impl OnnxGraphBuilder {
    pub(crate) fn node_gen(&mut self, model_proto: &ModelProto) {
        ...
        self.io = remove_unused_graph_inputs(&graph_io);
    }
            ...
            for input in node.inputs.iter_mut().skip(1) {
                ...
                if let Some(const_idx) = self.constants_map.get(input) {
                        ...
                        && graph_io.get_value(&constant.inputs[0]).is_some()
                    {
                        ...
                        graph_io.copy_value_type(input, &constant.inputs[0])
                    } else {
    ...
    fn handle_identity(&mut self, node: &mut Node, i: usize, graph_io: &OnnxGraphIO) {
        if node.node_type == NodeType::Identity && graph_io.get_value(&node.inputs[0]).is_none() {
            ...
            // since the node args are now just keys, where the key is equal to the orginal name
...
fn remap_unsqueeze_to_reshape(node: &mut Node, graph_io: &mut OnnxGraphIO) {
    ...
    match graph_io.get_type(&node.outputs[0]) {
        ...
        //and then overwriting the second one with the first. Unecessary work at best
...
fn rename_io(node: &mut Node, graph_io: &mut OnnxGraphIO) {
    ...
    for node_input in node.inputs.iter_mut() {
        if let Some(input_name) = graph_io.get_new_name(&node_input) {
            graph_io.set_passed(&input_name);
            //node_input.name.clone_from(&input_name);
        } else {
            //what requires name to be empty?
            graph_io.update_name(node_input, "");
...
fn remove_unused_graph_inputs(graph_io: &OnnxGraphIO) -> OnnxGraphIO {
    let mut new_graph_io = OnnxGraphIO::default();
    ...
    graph_io
        .old_io_names
        .iter()
        .for_each(|(old_name, io_entry)| {
            match io_entry {
                IOEntry::In(i) => {
                    if graph_io.inputs[*i].passed {
                        new_graph_io
                            .old_io_names
                            .insert(old_name.to_owned(), IOEntry::In(new_graph_io.inputs.len()));
                        new_graph_io.inputs.push(graph_io.inputs[*i].clone());
                    }
                }
                IOEntry::Out(i) => {
                    if graph_io.outputs[*i].passed {
                        new_graph_io.old_io_names.insert(
                            old_name.to_owned(),
                            IOEntry::Out(new_graph_io.outputs.len()),
                        );
                        new_graph_io.outputs.push(graph_io.outputs[*i].clone());
                    }
                }
                IOEntry::Node(i) => {
                    //if graph_io.node_out[i].passed {
                    new_graph_io.old_io_names.insert(
                        old_name.to_owned(),
                        IOEntry::Node(new_graph_io.node_out.len()),
                    );
                    new_graph_io.node_out.push(graph_io.node_out[*i].clone());
                }
            }
        });
    new_graph_io
