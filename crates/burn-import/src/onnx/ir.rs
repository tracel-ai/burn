...
/// ONNX graph representation. input and output are strictly graph inputs and outputs, and not any intermediate values.
#[derive(Debug, Clone)]
...
/// Nodes in a format used for the parsing process
#[derive(Debug, Clone)]
...
impl PartialEq for ConversionNode {
...
// Required by topological sort
impl Eq for ConversionNode {}

// Required by topological sort
impl core::hash::Hash for ConversionNode {
...
#[derive(Debug, Clone)]
pub struct Node {
    /// The type of the node, which should be a valid ONNX operator.
    pub node_type: NodeType,
    pub name: String,
    pub inputs: Vec<Argument>,
    pub outputs: Vec<Argument>,
...
impl Node {
    pub fn new(node: ConversionNode, graph_io: &OnnxGraphIO) -> Self {
        let inputs = node
            .inputs
            .iter()
            .map(|name| graph_io.get_arg(name).clone())
            .collect();
