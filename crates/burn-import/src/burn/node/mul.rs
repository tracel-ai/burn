...
impl<PS: PrecisionSettings> NodeCodegen<PS> for MulNode {
    ...
    fn forward(&self, scope: &mut Scope, node_position: usize) -> TokenStream {
        ...
        let function = match (&self.lhs, &self.rhs) {
            ...
            (Type::Tensor(_), Type::Scalar(_)) => quote! { #lhs.mul_scalar(#rhs) },
            (Type::Scalar(_), Type::Tensor(_)) => quote! { #rhs.mul_scalar(#lhs) },
...
impl OnnxIntoNode for MulNode {
    fn from_onnx(node: onnx_ir::Node) -> Self {
        let onnx_ir::Node::Mul(n) = node else {
        ...
        let expected = quote! {
            ...
            impl<B: Backend> Model<B> {
                ...
                pub fn forward(&self, tensor1: Tensor<B, 4>, tensor2: Tensor<B, 4>) -> Tensor<B, 4> {
                    let tensor3 = tensor1.mul(tensor2);
