...
impl<PS: PrecisionSettings> NodeCodegen<PS> for BitwiseXorNode {
    ...
    fn input_types(&self) -> Vec<Type> {
        self.inputs.iter().map(|t| Type::Tensor(t.clone())).collect()
    }

    fn forward(&self, scope: &mut Scope, node_position: usize) -> TokenStream {
        let inputs = self.inputs.iter().map(|t| scope.tensor_use_owned(t, node_position)).collect::<Vec<_>>();
...
mod tests {
    ...
    fn test_codegen_bitwise_or() {
        ...
        graph.register(BitwiseXorNode {
            inputs: vec![TensorType::new_float("input1", 4), TensorType::new_float("input2", 4)],
            output: TensorType::new_float("output", 4),
        });
        graph.register_input_output(vec!["input1".to_string(), "input2".to_string()], vec!["output".to_string()]);
