    ...
    fn test_codegen() {
        ...
        let expected = quote! {
            ...
            pub struct ModelSegmentedLayerLoader<B: Backend> {
                conv_transpose_2d: Option<ConvTranspose2d<B>>,
            ...
            impl<B: Backend> ModelSegmentedLayerLoader<B> {
                ...
                pub fn new(device: &B::Device) -> Self {
                    Self {
                        conv_transpose_2d: None,
                        phantom: core::marker::PhantomData,
                        device: burn::module::Ignored(device.clone()),
                    }
                }
                #[allow(unused)]
                pub fn unload_conv_transpose_2d(&mut self) {
                    self.conv_transpose_2d = None;
                }
                #[allow(unused)]
                pub fn load_conv_transpose_2d(&mut self, device: &B::Device) {
                    #[allow(clippy::useless_conversion)]
                    let record: <ConvTranspose2d<B> as burn::module::Module<B>>::Record = Self::recorder()
                        .load(CONV_TRANSPOSE_2D_STATES.into(), device)
                    ...
                    let conv_transpose_2d = ConvTranspose2dConfig::new([3, 3], [1, 1])
                        ...
                        .init(device);
                    self
                        .conv_transpose_2d = Some(
                        burn::module::Module::<B>::load_record(conv_transpose_2d, record),
                    );
                }
                ...
                pub fn memory_efficient_forward(
                    ...
                    self.load_conv_transpose_2d(device);
                    let output = self.conv_transpose_2d.take()?.forward(input);
                    Some(output)
                }
            }
            #[derive(Module, Debug)]
