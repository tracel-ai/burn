...
impl<PS: PrecisionSettings> NodeCodegen<PS> for onnx_ir::comparison::LessOrEqualNode {
    ...
    fn forward(&self, scope: &mut ScopeAtPosition<'_>) -> TokenStream {
        ...
        let function = match (&lhs.ty, &rhs.ty) {
            ...
            (ArgType::Shape(_), ArgType::Tensor(_)) => quote! {
                Tensor::<B, 1, burn::tensor::Int>::from_data(&#lhs_value as &[_], &*self.device).lower_equal(#rhs_value)
            },
            (ArgType::Tensor(_), ArgType::Shape(_)) => quote! {
                #lhs_value.lower_equal(Tensor::<B, 1, burn::tensor::Int>::from_data(&#rhs_value as &[_], &*self.device))
            },
...
mod tests {
    ...
    fn test_less_equal_forward() {
        ...
        assert_snapshot!(code, @"let output = lhs.lower_equal(rhs);");
    }
}
