...
def build_model():
    ...
    inner_then_graph = helper.make_graph(
        ...
        inputs=[
            ...
            helper.make_tensor_value_info('sum_init', TensorProto.FLOAT, [3]),
        ],
    ...
    # Inputs: [iter, cond, accum, cond2, sum_init]
    ...
    loop_body_graph = helper.make_graph(
        nodes=[loop_identity_x, loop_if, loop_identity_cond, loop_unsqueeze, loop_concat],
        name='loop_body',
        inputs=[
            helper.make_tensor_value_info('iter', TensorProto.INT64, []),
            helper.make_tensor_value_info('cond_in', TensorProto.BOOL, [1]),  # Rank-1
            helper.make_tensor_value_info('accum', TensorProto.FLOAT, [2, 3]),
            helper.make_tensor_value_info('cond2', TensorProto.BOOL, [1]),  # Rank-1 outer scope variable
            helper.make_tensor_value_info('sum_init', TensorProto.FLOAT, [3]),  # Outer scope variable
        ],
        outputs=[
            helper.make_tensor_value_info('cond_out', TensorProto.BOOL, [1]),  # Rank-1
    ...
    outer_then_loop = helper.make_node(
        'Loop',
        inputs=['iterations', 'cond_init', 'x_init', 'condition2', 'sum_init'],
        outputs=['loop_result'],
    ...
    outer_then_graph = helper.make_graph(
        ...
        inputs=[
            ...
            helper.make_tensor_value_info('iterations', TensorProto.INT64, [1]),  # Rank-1
            helper.make_tensor_value_info('cond_init', TensorProto.BOOL, [1]),  # Rank-1
            helper.make_tensor_value_info('sum_init', TensorProto.FLOAT, [3]),
        ],
        outputs=[
    ...
    main_graph = helper.make_graph(
        ...
        inputs=[
            ...
            helper.make_tensor_value_info('M', TensorProto.INT64, [1]),  # Rank-1 to avoid scalar issue
            helper.make_tensor_value_info('cond', TensorProto.BOOL, [1]),  # Rank-1 to avoid scalar issue
            helper.make_tensor_value_info('condition1', TensorProto.BOOL, [1]),  # Rank-1 to avoid scalar issue
            helper.make_tensor_value_info('condition2', TensorProto.BOOL, [1]),  # Rank-1 to avoid scalar issue
            helper.make_tensor_value_info('sum_init', TensorProto.FLOAT, [3]),
        ],
        outputs=[
...
def generate_test_data(model):
    ...
    sum_init = np.zeros(3, dtype=np.float32)
...
def main():
    ...
    print(f"sum_init: {test_data['sum_init'].tolist()}")
