...
def build_model():
    ...
    loop_body_graph = helper.make_graph(
        nodes=[loop_identity_x, loop_if, loop_identity_cond, loop_identity_out],
        name='loop_body',
        inputs=[
            helper.make_tensor_value_info('iter', TensorProto.INT64, []),
            helper.make_tensor_value_info('cond_in', TensorProto.BOOL, [1]),  # Rank-1 to match main graph
        ...
        outputs=[
            helper.make_tensor_value_info('cond_out', TensorProto.BOOL, [1]),  # Rank-1 to match input
    ...
    # Loop takes cond2 as outer scope variable (condition2 is from main graph)
    # No need for Identity, Loop will capture it from outer scope
