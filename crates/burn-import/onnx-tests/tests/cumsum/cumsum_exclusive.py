...
# used to generate model: onnx-tests/tests/cumsum/cumsum_exclusive.onnx
# Exclusive cumsum (exclusive=1, reverse=0)
# output[j] = sum of first (j-1) elements, excluding current element
...
# Create CumSum node (exclusive=1, reverse=0)
...
onnx.save(model, "cumsum_exclusive.onnx")
...
print(f"// Expected output (exclusive): [0., 1., 3., 6., 10.]")
print("ONNX model 'cumsum_exclusive.onnx' generated successfully.")
