...
def build_model():
    ...
    body_add = helper.make_node(
        'Add',
        inputs=['sum_state_in', 'input_element'],
        outputs=['sum_state_out']
    )

    # Also output the current sum (scan output)
    body_identity = helper.make_node(
        'Identity',
        inputs=['sum_state_out'],
        outputs=['scan_output']
    )
    ...
    body_graph = helper.make_graph(
        ...
        inputs=[
            # State variables
            helper.make_tensor_value_info('sum_state_in', TensorProto.FLOAT,
                                         [batch_size, feature_size]),
            # Scan inputs (one element from sequence)
            helper.make_tensor_value_info('input_element', TensorProto.FLOAT,
                                         [batch_size, feature_size]),
        ],
        outputs=[
            # State variables
            helper.make_tensor_value_info('sum_state_out', TensorProto.FLOAT,
                                         [batch_size, feature_size]),
            # Scan outputs
    ...
    graph = helper.make_graph(
        ...
        inputs=[
            helper.make_tensor_value_info('initial_sum', TensorProto.FLOAT,
                                         [batch_size, feature_size]),
            helper.make_tensor_value_info('input_sequence', TensorProto.FLOAT,
                                         [seq_len, batch_size, feature_size]),
        ],
        outputs=[
            helper.make_tensor_value_info('final_sum', TensorProto.FLOAT,
                                         [batch_size, feature_size]),
            helper.make_tensor_value_info('cumsum_sequence', TensorProto.FLOAT,
                                         [seq_len, batch_size, feature_size]),
        ],
    )
...
def generate_test_data(model):
    ...
    input_sequence1 = np.random.randn(seq_len, batch_size, feature_size).astype(np.float32)
    final_sum1, cumsum1 = sess.run(None, {
    ...
    input_sequence2 = np.random.randn(seq_len, batch_size, feature_size).astype(np.float32)
    final_sum2, cumsum2 = sess.run(None, {
    ...
    final_sum3, cumsum3 = sess.run(None, {
