...
# used to generate model: attention_attn_mask_int.onnx
...
def build_model():
    ...
    attn_mask = onnx.helper.make_tensor_value_info(
        "attn_mask", TensorProto.INT32, [2, 2]
    )
...
if __name__ == "__main__":
    ...
    attn_mask = np.array([[2, 0], [0, 3]], dtype=np.int32)
    onnx_model = build_model()
    file_name = "attention_attn_mask_int.onnx"
    ...
    session = ReferenceEvaluator("attention_attn_mask_int.onnx", verbose=1)
