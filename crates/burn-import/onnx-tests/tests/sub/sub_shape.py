...
# used to generate model: onnx-tests/tests/sub/sub_shape.onnx
...
def main():
    ...
    # Input tensors
    input_tensor1 = helper.make_tensor_value_info('input1', TensorProto.FLOAT, [10, 8, 6])
    ...
    # Subtract scalar from shape (Shape - Scalar)
    sub_scalar_node = helper.make_node(
        'Sub',
        inputs=['shape1', 'scalar'],
        outputs=['shape_minus_scalar']
    )
    
    # Subtract shape from shape (Shape - Shape)
    sub_shapes_node = helper.make_node(
        'Sub',
        inputs=['shape1', 'shape2'],
        outputs=['shape_minus_shape']
    )
    
    # Outputs - shape arrays
    output1 = helper.make_tensor_value_info('shape_minus_scalar', TensorProto.INT64, [3])
    output2 = helper.make_tensor_value_info('shape_minus_shape', TensorProto.INT64, [3])
    
    # Create the graph
    graph_def = helper.make_graph(
        [shape_node1, shape_node2, scalar_const, sub_scalar_node, sub_shapes_node],
        'shape_sub_test',
    ...
    shape_minus_scalar, shape_minus_shape = outputs
    
    print(f"\nTest output shape_minus_scalar: {repr(shape_minus_scalar)}")
    print(f"Test output shape_minus_shape: {repr(shape_minus_shape)}")
