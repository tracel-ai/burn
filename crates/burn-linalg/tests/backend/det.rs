use super::*;
use burn_core::tensor::{Distribution, Tolerance, s};
use burn_linalg::det;
...
fn test_det_large_diagonal() {
    ...
    let eye = Tensor::<2>::eye(8, &device);
    let values = Tensor::arange(1..9, &device).float();
...
fn test_det_mixed_singular_non_singular_batch() {
    let device = Default::default();
    let mut singular_tensor = Tensor::<2>::random([10, 10], Distribution::Default, &device);
    singular_tensor = singular_tensor.slice_fill(s![.., 8], 0.0);
    let identity_tensor = Tensor::<2>::eye(10, &device);
    let input_tensor = Tensor::stack(vec![singular_tensor, identity_tensor], 0);
    let det_tensor = det::<3, 2, 1>(input_tensor);
...
fn test_det_4d_square() {
    ...
    let det_tensor = det::<4, 3, 2>(tensor);
...
fn test_det_4d_rectangular_batch() {
    ...
    let det_tensor = det::<4, 3, 2>(tensor.clone());
...
fn test_det_product_property() {
    ...
    let det_a = det::<3, 2, 1>(a);
    let det_b = det::<3, 2, 1>(b);
    let det_c = det::<3, 2, 1>(c);
...
fn test_det_transpose_invariance() {
    ...
    let det_original = det::<3, 2, 1>(tensor.clone());
    let det_transpose = det::<3, 2, 1>(tensor.transpose());
    let tolerance = Tolerance::default().set_half_precision_absolute(5e-3);
    det_original
        .into_data()
...
fn test_det_f16_dtype_roundtrip_1x1() {
    ...
    let det_tensor = det::<3, 2, 1>(tensor.clone());
