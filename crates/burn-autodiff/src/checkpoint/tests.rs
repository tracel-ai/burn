use burn_tensor::{Bool, Data};
use burn_wgpu::AutoGraphicsApi;

use crate::Autodiff;

use super::strategy::BalancedCheckpointing;
...
pub type TestAutodiffTensor = burn_tensor::Tensor<TestAutodiffBackend, 2>;
pub type TestBoolTensor = burn_tensor::Tensor<TestAutodiffBackend, 2, Bool>;

// Does not save its state and does not need its parents
fn memory_bound_eager(
    tensor_a: TestAutodiffTensor,
    tensor_b: TestAutodiffTensor,
) -> TestAutodiffTensor {
    tensor_a.add(tensor_b)
}
fn memory_bound_eager_scalar(tensor_a: TestAutodiffTensor, b: f32) -> TestAutodiffTensor {
    tensor_a.add_scalar(b)
}

// Saves its own state and does not need its parents
fn compute_bound_eager(
    tensor_a: TestAutodiffTensor,
    tensor_b: TestAutodiffTensor,
) -> TestAutodiffTensor {
    let mask = TestBoolTensor::from_bool(
        Data::from([[true, false], [true, false]]),
        &tensor_a.device(),
    );
    tensor_a.mask_where(mask, tensor_b)
}
fn compute_bound_eager_scalar(tensor_a: TestAutodiffTensor, b: f32) -> TestAutodiffTensor {
    let mask = TestBoolTensor::from_bool(
        Data::from([[true, false], [true, false]]),
        &tensor_a.device(),
    );
...
fn assert_checkpoint(tensor: TestAutodiffTensor) {
