...
mod tests {
    ...
    fn test_lu_2x2_decomposition() {
        ...
        let expected = TestTensor::<2>::from_data(
            ...
            [2.0/3.0, 1.0]], 
            &device);
        assert_eq!(result.to_data(), expected.to_data(), "LU decomposition result does not match expected value");
    }
    ...
    fn test_lu_3x3_decomposition() {
        ...
        assert_eq!(permutations.to_data().to_vec::<i64>().unwrap(), vec![1, 2, 0], "LU decomposition permutations do not match expected value");
        assert_eq!(result.to_data(), expected.to_data(), "LU decomposition result does not match expected value");
    }
    ...
    fn test_lu_singular_matrix() {
        ...
        assert!(result.is_err(), "LU decomposition should fail for singular matrices");
    }
    ...
    fn test_lu_non_square_matrix() {
        ...
        let result = lu_decomposition(tensor);
        assert!(result.is_err(), "LU decomposition should fail for non-square matrices");
    }
    ...
    fn test_lu_1x1_element_matrix() {
        ...
        assert_eq!(result.to_data(), expected.to_data(), "LU decomposition result does not match expected value for 1x1 matrix");
    }
}
