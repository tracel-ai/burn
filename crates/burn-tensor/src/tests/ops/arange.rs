...
mod tests {
    ...
    fn test_arange() {
        ...
        assert_eq!(tensor.into_data(), TensorData::from([2, 3, 4]));

        // Test arange with negative numbers
        let tensor = Tensor::<TestBackend, 1, Int>::arange(-10..-5, &device);
        assert_eq!(tensor.into_data(), TensorData::from([-10, -9, -8, -7, -6]));

        let tensor = Tensor::<TestBackend, 1, Int>::arange(-3..0, &device);
        assert_eq!(tensor.into_data(), TensorData::from([-3, -2, -1]));
