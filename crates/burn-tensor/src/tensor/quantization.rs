use crate::{Tensor, ops::PrimitiveKind};
...
pub fn compute_range<const D: usize>(
    ...
    let (min, max) = match &tensor.primitive {
        ...
        PrimitiveKind::QFloat(_) => unreachable!(),
        _ => panic!("Should be Float primitive kind"),
    };

    CalibrationRange {
        min: Tensor::from_primitive(PrimitiveKind::Float(min)),
        max: Tensor::from_primitive(PrimitiveKind::Float(max)),
    }
}
...
pub fn compute_q_params(scheme: &QuantScheme, range: CalibrationRange) -> QuantizationParameters {
    match (range.min.primitive, range.max.primitive) {
        (PrimitiveKind::Float(min), PrimitiveKind::Float(max)) => {
            let qparams = quantization::compute_q_params::<Dispatch>(scheme, min, max);
            QuantizationParameters {
                scales: Tensor::from_primitive(PrimitiveKind::Float(qparams.scales)),
            }
        }
        _ => unreachable!(),
    }
