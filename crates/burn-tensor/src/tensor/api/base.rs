...
use crate::{Bool, Float, Shape, TensorData, TensorKind, backend::Backend, check};
use crate::{DType, Element};
use crate::{cast::ToElement, check::TensorCheck};

use super::{BasicOps, Slice, TensorMetadata};
    ...
    K: BasicOps<B>,
{
    ...
    ///
    /// # NOTE
    /// This method returns a view if conditions are compatible with the shape provided. Otherwise, it returns a copy.
    ...
    fn fmt_inner_tensor(
        ...
        for i in start..end {
            ...
            if let Some(data) = data {
                let elem = data.iter::<<K as TensorKind<B>>::Elem>().next().unwrap();
