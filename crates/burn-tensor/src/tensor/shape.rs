...
impl Shape {
    ...
    pub fn slice<const D: usize, S>(self, slices: S) -> [Slice; D]
    where
        S: SliceArg<D>,
    {
...
mod tests {
    ...
    #[test]
    fn test_slice() {
        let slices = Shape::new([3]).slice(1..4);
        assert_eq!(slices[0].to_range(3), 1..3);
        ...
        let slices = Shape::new([2, 3, 4]).slice(s![..20, 2]);
        assert_eq!(slices[0].to_range(2), 0..2);
