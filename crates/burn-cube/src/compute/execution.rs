...
impl<R: Runtime> KernelLauncher<R> {
    pub fn add_tensor(&mut self, tensor: &TensorHandle<'_, R>) {
        self.tensors.push(tensor);
    }

    pub fn add_scalar_u32(&mut self, scalar: u32) {
        if !self.scalar_order.contains(&Elem::UInt) {
            self.scalar_order.push(Elem::UInt);
        }
