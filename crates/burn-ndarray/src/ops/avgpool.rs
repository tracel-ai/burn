...
pub(crate) fn avg_pool3d<E: FloatNdArrayElement>(
    x: SharedArray<E>,
    kernel_size: [usize; 3],
    stride: [usize; 3],
    padding: [usize; 3],
    ...
    let [batch_size, channels, x_depth, x_height, x_width] = x.shape().try_into().unwrap();
            ...
            for od in 0..out_depth {
                for oh in 0..out_height {
                    for ow in 0..out_width {
                            ...
                            for kh in 0..kernel_height {
                                ...
                                for kw in 0..kernel_width {
                                    ...
                                    if id < padded_depth && ih < padded_height && iw < padded_width
                                    {
                                        ...
                                        // Check if within valid (non-padding) input bounds
                                        if id >= padding_depth
                                            && id < x_depth + padding_depth
                                            && ih >= padding_height
                                            && ih < x_height + padding_height
                                            && iw >= padding_width
                                            && iw < x_width + padding_width
                                        {
                                            let id_valid = id - padding_depth;
                                            let ih_valid = ih - padding_height;
                                            let iw_valid = iw - padding_width;
                                            sum_val += x[[b, c, id_valid, ih_valid, iw_valid]];
                                            valid_count += 1;
                                        }
                                    }
                                }
                        ...
                        output[[b, c, od, oh, ow]] = if count == 0 {
                            0.elem()
                        } else {
                            sum_val / (count as i32).elem()
                        };
                    }
                }
            }
...
pub(crate) fn avg_pool3d_backward<E: FloatNdArrayElement>(
    x: SharedArray<E>,
    grad: SharedArray<E>,
    kernel_size: [usize; 3],
    stride: [usize; 3],
    ...
    let [padding_depth, padding_height, padding_width] = padding;
    let [batch_size, channels, x_depth, x_height, x_width] = x.shape().try_into().unwrap();
    let [_batch_size, _channels, out_depth, out_height, out_width] =
    ...
    let mut output_grad =
        Array5::from_elem((batch_size, channels, x_depth, x_height, x_width), 0.elem());
    ...
    for b in 0..batch_size {
        for c in 0..channels {
            for od in 0..out_depth {
                for oh in 0..out_height {
                    for ow in 0..out_width {
                        let id_start_kernel = od * stride_depth;
                        let ih_start_kernel = oh * stride_height;
                        let iw_start_kernel = ow * stride_width;

                        let id_end_kernel = id_start_kernel + kernel_depth;
                        let ih_end_kernel = ih_start_kernel + kernel_height;
                        let iw_end_kernel = iw_start_kernel + kernel_width;

                        // Clip to valid input bounds (for gradient distribution)
                        let id_start = usize::max(id_start_kernel, padding_depth);
                        let ih_start = usize::max(ih_start_kernel, padding_height);
                        let iw_start = usize::max(iw_start_kernel, padding_width);
                        let id_end = usize::min(id_end_kernel, x_depth + padding_depth);
                        let ih_end = usize::min(ih_end_kernel, x_height + padding_height);
                        let iw_end = usize::min(iw_end_kernel, x_width + padding_width);
                        ...
                        let count = if count_include_pad {
                            // Count positions within padded bounds (not ceil_mode extensions)
                            let id_start_padded = id_start_kernel;
                            let ih_start_padded = ih_start_kernel;
                            let iw_start_padded = iw_start_kernel;
                            let id_end_padded = usize::min(id_end_kernel, padded_depth);
                            let ih_end_padded = usize::min(ih_end_kernel, padded_height);
                            let iw_end_padded = usize::min(iw_end_kernel, padded_width);
                            (id_end_padded.saturating_sub(id_start_padded))
                                * (ih_end_padded.saturating_sub(ih_start_padded))
                                * (iw_end_padded.saturating_sub(iw_start_padded))
                        } else {
                            // Count only valid (non-padding) positions
                        ...
                        let grad_val = grad[[b, c, od, oh, ow]] / (count as i32).elem();

                        for id in id_start..id_end {
                            for ih in ih_start..ih_end {
                                for iw in iw_start..iw_end {
                                    let id = id - padding_depth;
                                    let ih = ih - padding_height;
                                    let iw = iw - padding_width;

                                    output_grad[[b, c, id, ih, iw]] += grad_val;
                                }
                            }
                        }
                    }
