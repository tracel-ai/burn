...
impl<E: FloatNdArrayElement> IntTensorOps<Self> for NdArray<E> {
    fn int_from_dyn<const D: usize>(dyn_tensor: <Self as Backend>::DynTensorPrimitive) -> IntTensor<Self, D> {
        match dyn_tensor {
            ...
            _ => panic!("cannot create int tensor from non-int dyn tensor")
        }
    }
    ...
    fn int_into_data<const D: usize>(tensor: IntTensor<Self, D>) -> Reader<Data<i64, D>> {
    ...
    fn int_neg<const D: usize>(tensor: IntTensor<Self, D>) -> IntTensor<Self, D> {
    ...
    fn int_sum<const D: usize>(tensor: IntTensor<Self, D>) -> NdArrayTensor<i64, 1> {
    ...
    fn int_mean<const D: usize>(tensor: IntTensor<Self, D>) -> NdArrayTensor<i64, 1> {
