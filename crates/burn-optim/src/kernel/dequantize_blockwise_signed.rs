//! CubeCL implementation of blockwise signed dynamic dequantization.
//!
//! Inverse of `quantize_blockwise_signed`. One thread per output element.
//! Each thread reads its byte from the packed codes (4 codes per u32),
//! reads the block's scale, decodes, multiplies, writes f32.
...
pub fn dequantize_blockwise_signed_kernel(
    codes_in: &Array<u32>,
    scales_in: &Array<f32>,
    ...
    // Locate this thread's packed word and which byte slot in it.
    // Packed word index in the codes_in array.
    let pack_idx = block_idx * PACKED_PER_BLOCK + (tid / PACK_FACTOR);
    let slot = tid % PACK_FACTOR;

    // Extract the right byte. Lowest-indexed code is in the low byte, so
...
pub fn dequantize_blockwise_signed_via_kernel<R: Runtime>(
    ...
    unsafe {
        dequantize_blockwise_signed_kernel::launch_unchecked::<R>(
            client,
            cube_count,
            cube_dim,
...
mod tests {
    ...
    fn dequantize_blockwise_signed_roundtrip() {
        ...
        let recovered = dequantize_blockwise_signed_via_kernel::<CudaRuntime>(
            &client,
            &q.codes,
            &q.scales,
            q.original_len,
        ...
        // Per-block absmax scaling spreads error: with absmax ≈ 0.9 and signed
        // dynamic encoding, depths 0-3 dominate (4+ frac bits) and relative
        // error is well under 5%. The full pipeline (normalize → encode →
        // decode → rescale) is what matters; per-element drift is small.
    ...
    #[cfg(feature = "test-cuda")]
    #[test]
    fn dequantize_blockwise_signed_matches_reference() {
        ...
        // Kernel path: quantize and dequantize via kernels.
        let q = quantize_blockwise_signed_via_kernel::<CudaRuntime>(&client, &data);
        let kernel_recovered = dequantize_blockwise_signed_via_kernel::<CudaRuntime>(
            &client,
            &q.codes,
            &q.scales,
            q.original_len,
        ...
        // Reference path: same operation via the Burn-tensor impl.
        let nd_device = Default::default();
        let input_tensor =
            Tensor::<NdArray, 1>::from_floats(data.as_slice(), &nd_device).reshape([2, BLOCK_SIZE]);
        let q_ref = quantize_blockwise(input_tensor, BLOCK_SIZE, signed_dynamic::encode);
        let dequant_ref = dequantize_blockwise(q_ref, BLOCK_SIZE, signed_dynamic::decode);
        let ref_recovered: Vec<f32> = dequant_ref.into_data().to_vec().unwrap();

        // Compare kernel-recovered against reference-recovered. Allow 5%
        // relative tolerance — same as the roundtrip test, accommodating
        // both encoding granularity and any FMA drift between paths.
