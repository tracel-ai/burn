...
mod tests {
    ...
    fn step_yields_constant_default_lr() {
        let mut scheduler = LrPolicyScheduler::new(0.01_f64);
        for _ in 0..3 {
            check_approx(scheduler.step().default(), 0.01);
        }
    }
    ...
    fn step_advances_linear_default_scheduler() {
        ...
        for expected_lr in expected {
            check_approx(scheduler.step().default(), expected_lr);
        }
    }
    ...
    fn save_load_preserves_default_scheduler_state() {
        ...
        for _ in 0..4 {
            check_approx(restored.step().default(), truth.step().default());
        }
    }
