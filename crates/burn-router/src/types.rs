use alloc::sync::Arc;
use burn_ir::{BackendRepr, OperationRepr, TensorHandle, TensorId, TensorRepr};
use burn_tensor::{
...
macro_rules! impl_multi_backend_types {
    ...
    ($module_name:ident, $DefaultBackend:ident, $($OtherBackend:ident),+) => {
        ...
        pub mod $module_name {
            ...
            pub enum Handle<$DefaultBackend: BackendRepr, $($OtherBackend: BackendRepr),+> {
                #[allow(missing_docs)]
            ...
            #[derive(Clone)]
            pub enum MultiRunnerClient<$DefaultBackend: BackendRepr, $($OtherBackend: BackendRepr),+> {
                #[allow(missing_docs)]
            ...
            impl<$DefaultBackend: BackendRepr, $($OtherBackend: BackendRepr),+> RunnerClient for MultiRunnerClient<$DefaultBackend, $($OtherBackend),+>
            {
               type Device = MultiDevice<$DefaultBackend, $($OtherBackend),+>;
                ...
                async fn read_tensor(&self, tensor: TensorRepr) -> TensorData {
                    match self {
            ...
            impl<$DefaultBackend: BackendRepr, $($OtherBackend: BackendRepr),+, Br> RunnerChannel for DirectChannel<($DefaultBackend, $($OtherBackend),+), Br>
            where
            ...
            impl<$DefaultBackend: BackendRepr, $($OtherBackend: BackendRepr),+> MultiBackendBridge for ByteBridge<($DefaultBackend, $($OtherBackend),+)> {
