...
pub struct LpipsVgg<B: Backend> {
    ...
    extractor: VggFeatureExtractor<B>,
...
pub struct LpipsAlex<B: Backend> {
    ...
    extractor: AlexFeatureExtractor<B>,
    /// Linear layers for each feature level
    lin0: Conv2d<B>,
    lin1: Conv2d<B>,
    lin2: Conv2d<B>,
...
pub struct LpipsSqueeze<B: Backend> {
    ...
    extractor: SqueezeFeatureExtractor<B>,
    /// Linear layers for each feature level
    lin0: Conv2d<B>,
    lin1: Conv2d<B>,
    lin2: Conv2d<B>,
...
impl<B: Backend> LpipsVgg<B> {
    /// Compute LPIPS loss without reduction using VGG backbone.
...
impl<B: Backend> LpipsAlex<B> {
    /// Compute LPIPS loss without reduction using AlexNet backbone.
    pub fn forward_no_reduction(&self, input: Tensor<B, 4>, target: Tensor<B, 4>) -> Tensor<B, 1> {
        let [batch, _, _, _] = input.dims();
...
impl<B: Backend> LpipsSqueeze<B> {
    /// Compute LPIPS loss without reduction using SqueezeNet backbone.
    pub fn forward_no_reduction(&self, input: Tensor<B, 4>, target: Tensor<B, 4>) -> Tensor<B, 1> {
        let [batch, _, _, _] = input.dims();
...
impl<B: Backend> ModuleDisplay for Lpips<B> {
    ...
    fn custom_content(&self, content: Content) -> Option<Content> {
        ...
        content
            .add("net", &alloc::format!("{}", net_name))
            .add("normalize", &alloc::format!("{}", normalize))
            .optional()
    }
}
