...
/// `soft_shrink(x) = x - bias if x > lambd, x + bias if x < -lambd, 0 otherwise`
///
/// Should be created with [SoftShrinkConfig](SoftShrinkConfig).
#[derive(Module, Clone, Debug)]
#[module(custom_display)]
pub struct SoftShrink {
    /// The lambd value for the Soft Shrink formulation.
    pub lambd: f64,
    /// The bias value for the Soft Shrink formulation.
    // Usually bias = lambd, but need this to handle onnx spec https://onnx.ai/onnx/operators/onnx__Shrink.html
    pub bias: f64,
}
...
pub struct SoftShrinkConfig {
    /// The lambd value for the Soft Shrink formulation. Default is 0.5
    #[config(default = "0.5")]
    pub lambd: f64,
    /// The bias value for the Soft Shrink formulation. Default is equal to lambd.
    #[config(default = "0.5")]
    pub bias: f64,
}
...
impl SoftShrink {
    ...
    pub fn forward<B: Backend, const D: usize>(&self, input: Tensor<B, D>) -> Tensor<B, D> {
        soft_shrink(input, self.lambd, self.bias)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_soft_shrink_with_lambd_and_bias() {
        let device = <TestBackend as Backend>::Device::default();
        let model: SoftShrink = SoftShrinkConfig::new()
            ...
            .init();
        let input = Tensor::<TestBackend, 2>::from_floats(
            TensorData::from([[0.125, -0.125, -0.5], [0.75, 0.1, 0.0]]),
            &device,
        );
        let out = model.forward(input);
