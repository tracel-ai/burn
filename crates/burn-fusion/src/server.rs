    ...
    R: FusionRuntime,
{
        ...
        B: FusionBackend<FusionRuntime = R>,
    {
        ...
        let tensor = self.handles.get_byte_tensor::<B>(&tensor);
        B::byte_into_data(tensor)
    }
    ...
    pub fn resolve_server_byte<B>(&mut self, tensor: &TensorDescription) -> B::ByteTensorPrimitive
    where
        B: FusionBackend<FusionRuntime = R>,
    {
        self.handles.get_byte_tensor::<B>(tensor)
    }
    ...
    {
        let tensor = self.handles.get_byte_tensor::<B>(tensor);
        let tensor = B::byte_to_device(tensor, device);
        let id = server_device.create_empty_handle();

        server_device
            .handles
            .register_byte_tensor::<B>(&id, tensor.clone());
