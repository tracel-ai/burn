    ...
    R: FusionRuntime + 'static,
{
    ...
    ///
    /// With cross-stream tensor sharing handled via [`OperationIr::SharedView`], every input
    /// tensor in `repr` is assumed to live on `stream`. Pass `StreamId::current()` for normal ops
    /// and the tensor's own stream for `Drop`.
    pub fn register<O>(
        &self,
        stream: StreamId,
        repr: OperationIr,
