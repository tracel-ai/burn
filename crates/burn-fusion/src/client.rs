    ...
    R: FusionRuntime + 'static,
{
    ...
    ///
    /// Cross-stream tensor sharing is resolved out-of-band by [`Self::tag_shared_view`] before
    /// this call, so every input tensor in `repr` is assumed to live on `stream`. Pass
