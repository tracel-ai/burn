...
#[allow(missing_docs)]
#[macro_export(local_inner_macros)]
macro_rules! binary_byte_cmp_ops {
    (
        $name:ident,
        $ops:expr
    ) => {
        ...
        impl<B: FusionBackend> Operation<B::FusionRuntime> for $name<B> {
            fn execute(self: Box<Self>, handles: &mut HandleContainer<B::Handle>) {
                let lhs = handles.get_byte_tensor::<B>(&self.desc.lhs);
                let rhs = handles.get_byte_tensor::<B>(&self.desc.rhs);
                let output = $ops(lhs, rhs);
