...
use burn_ir::{BackendRepr, OperationRepr, TensorHandle};
use burn_tensor::{
...
/// [tensor operations](OperationRepr) into one, improving the performance of the backend.
///
///
/// # Notes
///
...
pub trait OptimizationBuilder<O>: Send {
    /// Register a new [tensor operation](OperationRepr).
...
pub trait FusionBackend:
    BackendRepr<Handle = FusionHandle<Self::FusionRuntime>, Device = FusionDevice<Self::FusionRuntime>>
{
...
// Fusion implements `BackendRepr` to enable router backend usage.
impl<B: FusionBackend> BackendRepr for Fusion<B> {
