...
/// Owns the request channel on the service runtime and turns task batches into wire frames.
///
/// The runner thread hands raw [`Task`] batches to [`send`](Self::send) over a bounded
...
pub(crate) struct RequestWriter {
    ...
    tx: Option<mpsc::Sender<Vec<Task>>>,
...
impl RequestWriter {
    ...
    pub(crate) fn spawn<C: ProtocolClient>(
        ...
        let (tx, mut rx) = mpsc::channel::<Vec<Task>>(WRITE_QUEUE_CAP);
    ...
    pub(crate) fn send(&self, runtime: &tokio::runtime::Runtime, batch: Vec<Task>) {
        let Some(tx) = self.tx.as_ref() else {
            log::warn!("Remote request writer already shut down; dropping outgoing batch");
            return;
        };
        match tx.try_send(batch) {
            ...
            Err(mpsc::error::TrySendError::Full(batch)) => {
                ...
                if runtime.block_on(tx.send(batch)).is_err() {
                    log::warn!(
                        "Remote request writer task has exited (server disconnected?); dropping outgoing batch"
                    );
                }
            }
