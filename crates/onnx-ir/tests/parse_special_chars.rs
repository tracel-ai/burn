...
fn test_parse_model_with_special_chars() {
    ...
    let problematic_outputs = graph.nodes.iter().filter(|node| {
        node.outputs.iter().any(|output| {
            output.name.contains('/') || output.name.contains(':') || !is_valid_identifier(&output.name)
        })
    }).count();
    ...
    let problematic_inputs = graph.nodes.iter().filter(|node| {
        node.inputs.iter().any(|input| {
            input.name.contains('/') || input.name.contains(':') || !is_valid_identifier(&input.name)
        })
    }).count();
    ...
    let problematic_node_names = graph.nodes.iter().filter(|node| {
        node.name.contains('/') || node.name.contains(':') || !is_valid_identifier(&node.name)
    }).count();
    
    assert_eq!(
        problematic_node_names, 
        0, 
