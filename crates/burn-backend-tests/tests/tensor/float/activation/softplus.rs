...
fn test_softplus_large_values_do_not_overflow() {
    ...
    let output = activation::softplus(tensor, 5.0);
    let expected = TensorData::from([20.0, 50.0, 100.0, 1000.0]);
    output
        .into_data()
        .assert_approx_eq::<FloatElem>(&expected, Tolerance::default());
...
fn test_softplus_small_values_do_not_underflow() {
    ...
    let output = activation::softplus(tensor, 1.0);
...
fn test_softplus_with_threshold() {
    ...
    let output = activation::softplus_with_threshold(tensor.clone(), 1.0, 80.0);
    let expected = TensorData::from([5.0067153, 25.0]);
    output
        .into_data()
        .assert_approx_eq::<FloatElem>(&expected, Tolerance::default());
    ...
    let output = activation::softplus_with_threshold(tensor, 1.0, 1.0);
