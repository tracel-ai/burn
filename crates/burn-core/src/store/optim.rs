...
//! named tensors (suitable for the [burnpack](burn_pack) format) plus a few stringly-typed
//! scalars, and reconstructs it on load.
//!
//! Implementations are generated with `#[derive(OptimState)]`; see the derive for the supported
//! field shapes (`Tensor<D>`, `Option<Tensor<D>>`, scalars, nested states and optional nested
//! states).
...
impl OptimStateSink {
    ...
    pub fn push_scalar(&mut self, prefix: &str, leaf: &str, value: String) {
...
impl OptimStateSource {
    /// Create a source from an existing scalar map (e.g. the burnpack metadata).
    pub fn new(scalars: BTreeMap<String, String>) -> Self {
        Self {
    ...
    pub fn take_scalar(&mut self, prefix: &str, leaf: &str) -> Option<String> {
        self.scalars.get(&join_path(prefix, leaf)).cloned()
    }
}
