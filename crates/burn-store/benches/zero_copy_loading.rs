...
mod verification {
    ...
    fn verify_operations_on_static_data() {
        ...
        let sum: f32 = weight.clone().sum().to_data().try_into_vec().unwrap()[0];
        assert!(
            sum.is_finite(),
        ...
        let matmul_sum: f32 = matmul_result.sum().to_data().try_into_vec().unwrap()[0];
        assert!(
        ...
        let doubled_sum: f32 = doubled.sum().to_data().try_into_vec().unwrap()[0];
        assert!(
    ...
    fn verify_static_vs_bytes_equality() {
            ...
            .enumerate()
        {
            ...
            let sum_zc: f32 = weight_zc.clone().sum().to_data().try_into_vec().unwrap()[0];
            let sum_copy: f32 = weight_copy.clone().sum().to_data().try_into_vec().unwrap()[0];
            assert!(
