...
//!
//! The burnpack reader and writer operate on [`BurnpackTensor`] values, which
...
//! `burn-core`) bridge between [`BurnpackTensor`] and their own tensor/snapshot
//! types.
...
pub type TensorBytesFn = Rc<dyn Fn() -> Result<Bytes, BurnpackError>>;
...
impl BurnpackTensor {
    ...
    pub fn bytes(&self) -> Result<Bytes, BurnpackError> {
        (self.data)()
    }
}
