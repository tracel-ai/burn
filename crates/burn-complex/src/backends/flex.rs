use crate::base::{CBT, SplitTensorData};
use crate::utils::interleaved_data_to_imag_data;
use crate::{
...
use burn_flex::ops::binary::make_tensor;
use burn_flex::ops::binary::{
    binary_op_typed_convert, can_use_binary_inplace, scalar_op_typed_convert,
};
use burn_flex::ops::comparison::compare_elem_typed;
use burn_flex::ops::{
    binary::scalar_op_typed_rhs,
    comparison::{bool_scalar, compare_typed, iter_elements, reduce_bool_dim},
};

use burn_flex::strided_index::StridedIter;
use burn_flex::{Flex, FlexDevice, FlexTensor, ops::binary::scalar_op_typed};
impl CBT for Flex {
...
impl ComplexTensorOps<Flex> for Flex {
    ...
    fn complex_from_polar(
        ...
        let real_part = burn_flex::ops::binary::binary_op(
            magnitude.clone(),
            burn_flex::ops::unary::cos(phase.clone()),
            |m, cos_p| m * cos_p,
            |m, cos_p| m * cos_p,
            None,
        );
        let imag_part = burn_flex::ops::binary::binary_op(
            magnitude,
            burn_flex::ops::unary::sin(phase),
            |m, sin_p| m * sin_p,
            |m, sin_p| m * sin_p,
            None,
        );
    ...
    fn complex_equal(
        ...
        let (lhs, rhs) = burn_flex::ops::expand::broadcast_binary(lhs, rhs);
    ...
    fn complex_gather(
        ...
        match tensor.dtype() {
            DType::Complex32 => {
                burn_flex::ops::gather_scatter::gather::<Complex<f32>>(tensor, dim, indices)
            }
            DType::Complex64 => {
                burn_flex::ops::gather_scatter::gather::<Complex<f64>>(tensor, dim, indices)
            }
    ...
    fn complex_scatter_add(
        ...
        match tensor.dtype() {
            DType::Complex32 => burn_flex::ops::gather_scatter::scatter_add::<Complex<f32>>(
                tensor, dim, indices, values,
            ),
            DType::Complex64 => burn_flex::ops::gather_scatter::scatter_add::<Complex<f64>>(
                tensor, dim, indices, values,
            ),
            _ => panic!(
    ...
    fn complex_random(
        ...
        let mut seed = burn_flex::SEED.lock().unwrap();
        let mut rng = seed.take().unwrap_or_else(burn_flex::get_seeded_rng);
    ...
    fn complex_select(
        ...
        match tensor.dtype() {
            DType::Complex32 => {
                burn_flex::ops::gather_scatter::select::<Complex<f32>>(tensor, dim, indices)
            }
            DType::Complex64 => {
                burn_flex::ops::gather_scatter::select::<Complex<f64>>(tensor, dim, indices)
            }
    ...
    fn complex_select_add(
        ...
        match tensor.dtype() {
            DType::Complex32 => burn_flex::ops::gather_scatter::select_add::<Complex<f32>>(
                tensor, dim, indices, values,
            ),
            DType::Complex64 => burn_flex::ops::gather_scatter::select_add::<Complex<f64>>(
                tensor, dim, indices, values,
            ),
    ...
    fn complex_slice(tensor: ComplexTensor<Flex>, slices: &[Slice]) -> ComplexTensor<Flex> {
        burn_flex::ops::slice::slice(tensor, slices)
    }
    ...
    fn complex_slice_assign(
        ...
        burn_flex::ops::slice::slice_assign(tensor, ranges, value)
    }
    ...
    fn complex_repeat_dim(
        ...
        burn_flex::ops::repeat_dim::repeat_dim(tensor, dim, times)
    }

    fn complex_cat(tensors: Vec<ComplexTensor<Flex>>, dim: usize) -> ComplexTensor<Flex> {
        burn_flex::ops::cat::cat(tensors, dim)
    }
    ...
    fn complex_expand(tensor: ComplexTensor<Flex>, shape: burn_std::Shape) -> ComplexTensor<Flex> {
        burn_flex::ops::expand::expand(tensor, shape)
    }

    fn complex_flip(tensor: ComplexTensor<Flex>, axes: &[usize]) -> ComplexTensor<Flex> {
        burn_flex::ops::flip::flip(tensor, axes)
    }
    ...
    fn complex_mask_where(
        ...
        match tensor.dtype() {
            DType::Complex32 => {
                burn_flex::ops::mask::mask_where::<Complex<f32>>(tensor, mask, source)
            }
            DType::Complex64 => {
                burn_flex::ops::mask::mask_where::<Complex<f64>>(tensor, mask, source)
            }
    ...
    fn complex_mask_fill(
        ...
        match tensor.dtype() {
            DType::Complex32 => {
                burn_flex::ops::mask::mask_fill::<Complex<f32>>(tensor, mask, value)
            }
            DType::Complex64 => {
                ...
                burn_flex::ops::mask::mask_fill::<Complex<f64>>(tensor, mask, value64)
            }
    ...
    fn complex_matmul(lhs: ComplexTensor<Flex>, rhs: ComplexTensor<Flex>) -> ComplexTensor<Flex> {
        burn_flex::ops::matmul::matmul(lhs, rhs)
    }

    fn complex_cumsum(tensor: ComplexTensor<Flex>, dim: usize) -> ComplexTensor<Flex> {
        match tensor.dtype() {
            DType::Complex32 => burn_flex::ops::cumulative::cumsum::<Complex<f32>>(tensor, dim),
            DType::Complex64 => burn_flex::ops::cumulative::cumsum::<Complex<f64>>(tensor, dim),
    ...
    fn complex_cumprod(tensor: ComplexTensor<Flex>, dim: usize) -> ComplexTensor<Flex> {
        match tensor.dtype() {
            DType::Complex32 => burn_flex::ops::cumulative::cumprod::<Complex<f32>>(tensor, dim),
            DType::Complex64 => burn_flex::ops::cumulative::cumprod::<Complex<f64>>(tensor, dim),
    ...
    fn complex_scatter_nd(
        ...
        match tensor.dtype() {
            DType::Complex32 => burn_flex::ops::gather_scatter::scatter_nd_no_ord::<Complex<f32>>(
                tensor, indices, value, reduction,
            ),
            DType::Complex64 => burn_flex::ops::gather_scatter::scatter_nd_no_ord::<Complex<f64>>(
                tensor, indices, value, reduction,
            ),
    ...
    fn complex_gather_nd(
        ...
        match data.dtype() {
            DType::Complex32 => {
                burn_flex::ops::gather_scatter::gather_nd::<Complex<f32>>(data, indices)
            }
            DType::Complex64 => {
                burn_flex::ops::gather_scatter::gather_nd::<Complex<f64>>(data, indices)
            }
...
macro_rules! c2c_binary_op {
    ($lhs:expr, $rhs:expr, $op:expr) => {
        $crate::backends::flex::c2c_binary_op(
            $lhs,
            $rhs,
...
macro_rules! c2c_scalar_op {
    ($tensor:expr, $scalar:expr, $op:expr) => {
        $crate::backends::flex::c2c_scalar_op(
            $tensor,
            $scalar,
            |a: Complex<f32>, b: Complex<f32>| $op(a, b),
            |a: Complex<f64>, b: Complex<f64>| $op(a, b),
...
macro_rules! c2r_binary_op {
    ($lhs:expr, $rhs:expr, $op:expr) => {
        $crate::backends::flex::c2r_binary_op(
            $lhs,
            $rhs,
            |a: Complex<f32>, b: Complex<f32>| $op(a, b),
            |a: Complex<f64>, b: Complex<f64>| $op(a, b),
...
macro_rules! c2r_unary_op {
    ($tensor:expr, $op:expr) => {
        $crate::backends::flex::c2r_unary_op($tensor, $op, $op)
    };
}
...
macro_rules! c2c_unary_op {
    ($tensor:expr, $op:expr) => {
        $crate::backends::flex::c2c_unary_op($tensor, $op, $op)
    };
}
...
macro_rules! c2r_scalar_op {
    ($tensor:expr, $scalar:expr, $op:expr) => {
        $crate::backends::flex::c2r_scalar_op(
            $tensor,
            $scalar,
            |a: Complex<f32>, b: Complex<f32>| $op(a, b),
            |a: Complex<f64>, b: Complex<f64>| $op(a, b),
...
{
    use burn_flex::ops::binary::binary_op_typed;
...
{
    use burn_flex::ops::unary::unary_op_typed_convert;
...
{
    use burn_flex::ops::unary::unary_op_typed;
...
pub fn binary_op_typed_convert_rhs<E1, E2, O, Op>(
    lhs: FlexTensor,
    rhs: &FlexTensor,
    op: Op,
) -> FlexTensor
