...
use burn_tensor::{SliceInfo, TensorMetadata, backend::Backend};
...
fn slice_tensor<B: Backend>(
    ...
    let shape = tensor.shape();
    // full range across all dims as SliceInfo
    let full_range = shape
        .dims
        .iter()
        .map(|dim| SliceInfo::from_range(0..*dim))
        .collect::<Vec<SliceInfo>>();

    // Slice tensors
    let mut slices = vec![];
    for range in &slice_ranges {
        let mut all_ranges = full_range.clone();
        all_ranges[slice_dim] = SliceInfo::from_range(range.clone());
