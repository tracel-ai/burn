...
pub enum Exploration<'a, B: FusionBackend> {
    NewOptimization(Option<&'a dyn OptimizationBuilder<B>>),
    Continue,
}
...
impl<B: FusionBackend> Explorer<B> {
    ...
    pub(crate) fn explore<'a>(
        ...
        match find_best_optimization_index(&mut self.builders) {
            Some(index) => Exploration::NewOptimization(Some(self.builders[index].as_ref())),
            None => Exploration::NewOptimization(None),
        }
    }
