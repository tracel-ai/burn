use crate::{
  ...
  graph::{
    ...
    MaxPool2dWithIndicesDescription, Ops, TensorOpsDescription,
  },
...
impl<B: FusionBackend> ModuleOps<Fusion<B>> for Fusion<B> {
  ...
  fn avg_pool2d(
    ...
    let size_0 = calculate_pool_output_size(kernel_size[0], stride[0], padding[0], 1, x.shape[2]);
    let size_1 = calculate_pool_output_size(kernel_size[1], stride[1], padding[1], 1, x.shape[3]);
