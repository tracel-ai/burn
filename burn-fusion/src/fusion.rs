...
pub struct Fusion<Device, B, C, G> {
    clients: spin::Mutex<Option<HashMap<Device, FusionClient<B, C, G>>>>,
}

impl<Device, B, C, G> Fusion<Device, B, C, G>
where
    ...
    C: FusionChannel<B, G>,
{
    ...
    pub fn client(&self, device: &Device) -> FusionClient<B, C, G> {
        let mut clients = self.clients.lock();

        if clients.is_none() {
            let client = FusionClient::new(C::new(FusionServer::new()));
    ...
    fn register_inner(
        ...
        clients: &mut Option<HashMap<Device, FusionClient<B, C, G>>>,
    ) {
        if clients.is_none() {
