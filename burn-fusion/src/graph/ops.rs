...
impl<B: FusionBackend> ModuleOpsDescription<B> {
  ...
  fn execute(&self, handles: &mut HandleContainer<B>) {
    match self {
      ...
      ModuleOpsDescription::AdaptiveAvgPool1dBackward(desc, ops) => ops.execute(desc, handles),
      ModuleOpsDescription::AdaptiveAvgPool2dBackward(desc, ops) => ops.execute(desc, handles),
      ...
      ModuleOpsDescription::MaxPool1dWithIndicesBackward(desc, ops) => ops.execute(desc, handles),
      ...
      ModuleOpsDescription::MaxPool2dWithIndicesBackward(desc, ops) => ops.execute(desc, handles),
    }
  }
}
