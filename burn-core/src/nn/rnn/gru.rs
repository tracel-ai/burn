...
impl GruConfig {
  ...
  pub fn init_with<B: Backend>(self, record: GruRecord<B>) -> Gru<B> {
    ...
    Gru {
      update_gate: gate_controller::GateController::new_with(&linear_config, record.update_gate),
      reset_gate: gate_controller::GateController::new_with(&linear_config, record.reset_gate),
...
impl<B: Backend> Gru<B> {
  ...
  pub fn forward(&self, batched_input: Tensor<B, 3>, state: Option<Tensor<B, 3>>) -> Tensor<B, 3> {
