...
impl LstmConfig {
  ...
  pub fn init_with<B: Backend>(&self, record: LstmRecord<B>) -> Lstm<B> {
    ...
    Lstm {
      input_gate: gate_controller::GateController::new_with(&linear_config, record.input_gate),
      forget_gate: gate_controller::GateController::new_with(&linear_config, record.forget_gate),
      output_gate: gate_controller::GateController::new_with(&linear_config, record.output_gate),
...
mod tests {
  ...
  fn test_with_uniform_initializer() {
    ...
    let config =
      LstmConfig::new(5, 5, false).with_initializer(Initializer::Uniform { min: 0.0, max: 1.0 });
