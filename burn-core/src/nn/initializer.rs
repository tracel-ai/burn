...
impl Initializer {
  ...
  fn kaiming_std(&self, fan_out_only: bool, fan_in: Option<usize>, fan_out: Option<usize>) -> f64 {
    let fan = if fan_out_only { fan_out } else { fan_in };
    let fan =
      fan.expect("Can't use Kaiming initialization without specifying fan. Use init_with method.");
...
fn uniform_draw<B: Backend, const D: usize, S: Into<Shape<D>>>(
  ...
  let distribution = Distribution::Uniform(low.elem::<B::FloatElem>(), high.elem::<B::FloatElem>());
...
mod tests {
  ...
  fn initializer_xavier_uniform_init() {
    ...
    let tensor: Tensor<TB, 2> =
      Initializer::XavierUniform { gain }.init_with([fan_out, fan_in], Some(fan_in), Some(fan_out));
  ...
  fn initializer_xavier_normal_init() {
    ...
    let tensor: Tensor<TB, 2> =
      Initializer::XavierNormal { gain }.init_with([fan_out, fan_in], Some(fan_in), Some(fan_out));
    assert_normal_init(expected_mean, expected_var, &tensor)
  }
