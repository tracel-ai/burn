...
impl<B: Backend> TransformerDecoderLayer<B> {
  ...
  fn new_with(config: &TransformerDecoderConfig, record: TransformerDecoderLayerRecord<B>) -> Self {
    let self_attn = MultiHeadAttentionConfig::new(config.d_model, config.n_heads)
      .with_initializer(config.initializer.clone())
      .with_dropout(config.dropout)
      .with_quiet_softmax(config.quiet_softmax)
...
mod tests {
  ...
  fn test_autoregressive_norm_last() {
    ...
    test_autoregressive(
      TransformerDecoderConfig::new(d_model, d_ff, n_heads, num_layers).with_norm_first(false),
    )
  }
  ...
  fn test_autoregressive(config: TransformerDecoderConfig) {
    ...
    let memory =
      Tensor::<TestBackend, 3>::random([batch_size, seq_length, d_model], Distribution::Default);
    let target =
      Tensor::<TestBackend, 3>::random([batch_size, seq_length, d_model], Distribution::Default);
    let mask_attn = generate_autoregressive_mask(batch_size, seq_length, &target.device());
    let input =
      TransformerDecoderInput::new(target.clone(), memory.clone()).target_mask_attn(mask_attn);
