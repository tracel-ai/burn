...
impl<B: Backend> TransformerEncoderLayer<B> {
  fn new_with(config: &TransformerEncoderConfig, record: TransformerEncoderLayerRecord<B>) -> Self {
...
mod tests {
  ...
  fn test_autoregressive_norm_last() {
    ...
    test_autoregressive(
      TransformerEncoderConfig::new(d_model, d_ff, n_heads, num_layers).with_norm_first(false),
    )
  }
