...
use crate::{
  element::FloatNdArrayElement, iter_par, iter_range_par, ops::padding::apply_padding_4d, run_par,
          ...
          for ic in (in_channels * g)..(in_channels * (g + 1)) {
            ...
            for kh in 0..kernel_height {
              for kw in 0..kernel_width {
                ...
                if (1, 1, 1, 1) == (stride_width, stride_height, dilation_width, dilation_height) {
                  conv2d_mad_inner(
                    output.view_mut(),
                    x.view(),
                    k,
...
pub(crate) fn conv_transpose2d<E: FloatNdArrayElement>(
  ...
  let out_height =
    (in_height - 1) * stride_height + dilation_height * (kernel_height - 1) + out_padding_height
      - 2 * padding_height
      + 1;
  let out_width =
          ...
          for iw in 0..in_width {
            for kh in 0..kernel_height {
              for kw in 0..kernel_width {
                ...
                output[[b, oc_out, oh, ow]] += x[[b, ic, ih, iw]] * weight.array[[ic, oc, kh, kw]];
              }
            }
          }
        }
