...
impl<B: Backend> ModuleOps<Autodiff<B>> for Autodiff<B> {
      ...
      Some(bias) => {
          ...
          .stateful()
        {
          OpsKind::Tracked(prep) => prep.finish(
            ...
            B::conv_transpose2d(x.primitive, weight.primitive, Some(bias.primitive), options),
          ),
      ...
      Some(bias) => {
          ...
          .stateful()
        {
          OpsKind::Tracked(prep) => prep.finish(
            ...
            B::conv_transpose1d(x.primitive, weight.primitive, Some(bias.primitive), options),
          ),
    ...
    impl<B: Backend> Backward<B, 3, 1> for AvgPool1D {
      ...
      fn backward(self, ops: Ops<Self::State, 1>, grads: &mut Gradients) {
        ...
        if let Some(node) = node_parent {
          let grad =
            B::avg_pool1d_backward(x, grad, kernel_size, stride, padding, count_include_pad);
  ...
  fn max_pool1d(
    ...
    match MaxPool1D.prepare([x.node], [x.graph]).stateful() {
      OpsKind::Tracked(prep) => {
        let output =
          B::max_pool1d_with_indices(x.primitive.clone(), kernel_size, stride, padding, dilation);
        prep.finish(
          (
            x.primitive,
            output.indices,
  ...
  fn max_pool2d(
    ...
    match MaxPool2D.prepare([x.node], [x.graph]).stateful() {
      OpsKind::Tracked(prep) => {
        let output =
          B::max_pool2d_with_indices(x.primitive.clone(), kernel_size, stride, padding, dilation);
        prep.finish(
          (
            x.primitive,
            output.indices,
  ...
  fn adaptive_avg_pool1d(x: AutodiffTensor<B, 3>, output_size: usize) -> AutodiffTensor<B, 3> {
    ...
    match AdaptiveAvgPool1D.prepare([x.node], [x.graph]).stateful() {
      ...
      OpsKind::UnTracked(prep) => prep.finish(B::adaptive_avg_pool1d(x.primitive, output_size)),
    }
  }
  ...
  fn adaptive_avg_pool2d(x: AutodiffTensor<B, 4>, output_size: [usize; 2]) -> AutodiffTensor<B, 4> {
    ...
    match AdaptiveAvgPool2D.prepare([x.node], [x.graph]).stateful() {
      ...
      OpsKind::UnTracked(prep) => prep.finish(B::adaptive_avg_pool2d(x.primitive, output_size)),
    }
  }
