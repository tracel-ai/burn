...
impl TextGenerationModelConfig {
  pub fn init<B: Backend>(&self) -> TextGenerationModel<B> {
    ...
    let embedding_token = EmbeddingConfig::new(self.vocab_size, self.transformer.d_model).init();
    let embedding_pos = EmbeddingConfig::new(self.max_seq_length, self.transformer.d_model).init();
...
impl<B: Backend> TextGenerationModel<B> {
  pub fn forward_training(&self, item: TrainingTextGenerationBatch<B>) -> ClassificationOutput<B> {
