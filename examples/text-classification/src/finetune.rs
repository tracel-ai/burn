...
use burn::lr_scheduler::policy::{LrPolicyConfig, LrPolicyScheduler};
use burn::module::{LoraConfig, ParamGroup};
...
pub fn lora_finetuning<D: TextClassificationDataset + 'static>(
    ...
    // model.reset_head(D::num_classes());
    // model.transformer = model.transformer.no_grad();
    // model.embedding_pos = model.embedding_pos.no_grad();
    // model.embedding_token = model.embedding_token.no_grad();
...
// Do a full finetuning of the pre-trained weights.
pub fn full_finetuning<D: TextClassificationDataset + 'static>(
    strategy: ExecutionStrategy,
    dataset_train: D,         // Training dataset
    dataset_test: D,          // Testing dataset
    config: ExperimentConfig, // Experiment configuration
    ...
    // Load pre-trained weights.
    let mut model = model.load_file(&"model.bpk");
    ...
    // Initialize learning rate scheduler
    let mut head_group = ParamGroup::from_predicate("output");
    head_group.exclude(&ParamGroup::from_predicate("transformer"));
    let lr_scheduler = LrPolicyConfig::new(3e-5.into())
        .add_group(head_group, 1e-3)
        .init()
        .unwrap();
