...
impl TextClassificationModel {
    ...
    pub fn forward(&self, item: TextClassificationTrainingBatch) -> ClassificationOutput {
        ...
        let do_write = false;
        if do_write {
            // println!("mask_pad: {mask_pad}");
            use std::io::Write;
            let file = std::fs::OpenOptions::new()
                .create(true) // Create the file if it doesn't exist
                .append(true) // Append data to the end of the file (change to .write(true) to overwrite)
                .open("output.txt")
                .unwrap();
            let mut writer = std::io::BufWriter::new(file);
            let my_string = format!("mask_pad: {}", mask_pad);
            writeln!(writer, "{my_string}").unwrap();
            writer.flush().unwrap();
        }
