...
use burn::{
    ...
    train::{
        EvaluatorBuilder, ExecutionStrategy, Learner, MetricEarlyStoppingStrategy,
        StoppingCondition,
        metric::{
...
pub fn run2<B: AutodiffBackend>(strategy: ExecutionStrategy<B>, devices: Vec<B::Device>) {
    ...
    let dist_config = DistributedConfig {
        all_reduce_op: burn::tensor::backend::distributed::ReduceOperation::Mean,
    };
...
pub fn run<B: AutodiffBackend>(device: B::Device) {
    ...
    log::info!("model params : {}", model.num_params());
    log::info!("model params : {}", model.num_params());
