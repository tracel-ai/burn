...
impl Display for Operator {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      Operator::Add { lhs, rhs, out } => f.write_fmt(format_args!("let {out} = {lhs} + {rhs};")),
      Operator::Sub { lhs, rhs, out } => f.write_fmt(format_args!("let {out} = {lhs} - {rhs};")),
      Operator::Mul { lhs, rhs, out } => f.write_fmt(format_args!("let {out} = {lhs} * {rhs};")),
      Operator::Div { lhs, rhs, out } => f.write_fmt(format_args!("let {out} = {lhs} / {rhs};")),
      ...
      Operator::Tanh { input, out } => f.write_fmt(format_args!("let {out} = tanh({input});")),
      Operator::Erf { input, out } => f.write_fmt(format_args!("let {out} = erf({input});")),
      Operator::Recip { input, out } => f.write_fmt(format_args!("let {out} = 1.0 / {input};")),
      ...
      Operator::ReadGlobal {
        ...
        let (global, local) = match variable {
          Variable::Input(number) => (format!("input_{number}_global"), format!("input_{number}")),
