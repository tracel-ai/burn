...
mod tests {
  ...
  fn subsequent_calls_give_different_tensors() {
    ...
    let tensor_1 =
      Tensor::<TestBackend, 2>::random_device(shape.clone(), Distribution::Bernoulli(0.5), &device);
    let tensor_2 =
      Tensor::<TestBackend, 2>::random_device(shape.clone(), Distribution::Bernoulli(0.5), &device);
  ...
  fn number_of_1_proportional_to_prob() {
    ...
    assert!(
      f32::abs((bin_stats[1].count as f32 / shape.num_elements() as f32) - prob as f32) < 0.05
    );
  }
