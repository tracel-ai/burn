...
mod tests {
  ...
  fn subsequent_calls_give_different_tensors() {
    ...
    let tensor_1 = Tensor::<TestBackend, 2>::random_device(shape, Distribution::Default, &device);
    let tensor_2 = Tensor::<TestBackend, 2>::random_device(shape, Distribution::Default, &device);
    for i in 0..20 {
  ...
  fn at_least_one_value_per_bin_uniform() {
    ...
    let tensor =
      Tensor::<TestBackend, 2>::random_device(shape, Distribution::Uniform(-5., 10.), &device);
  ...
  fn runs_test() {
    ...
    let variance =
      ((2. * n_0 * n_1) * (2. * n_0 * n_1 - n_0 - n_1)) / ((n_0 + n_1).powf(2.) * (n_0 + n_1 - 1.));
