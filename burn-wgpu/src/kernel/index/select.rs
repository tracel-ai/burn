...
mod tests {
  ...
  fn select_should_work_with_multiple_workgroups() {
    ...
    let indices_ref = Tensor::<ReferenceBackend, 1, Int>::from_data(indices.to_data().convert());
  ...
  fn select_assign_same_as_ref<const D: usize>(dim: usize, shape: [usize; D]) {
    ...
    let indices = Tensor::<TestBackend, 1, Int>::from_data(
      Tensor::<TestBackend, 1>::random([shape[dim]], Distribution::Uniform(0., shape[dim] as f32))
        .into_data()
        .convert(),
    );
