...
pub(crate) fn scatter<E: WgpuElement, I: WgpuElement, const D: usize>(
  ...
  let kernel =
    StaticKernel::<KernelSettings<Scatter, E, i32, WORKGROUP_DEFAULT, WORKGROUP_DEFAULT, 1>>::new(
      elemwise_workgroup(num_elems_per_workgroup, WORKGROUP_DEFAULT),
    );
...
mod tests {
  ...
  fn same_as_reference_diff_shape<const D: usize>(
    ...
    let indices_ref = Tensor::<ReferenceBackend, D, Int>::from_data(indices.to_data().convert());
