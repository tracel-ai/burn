use crate::{
  ...
  element::WgpuElement,
  kernel::{build_info, elemwise_workgroup, KernelSettings, StaticKernelSource, WORKGROUP_DEFAULT},
  kernel_wgsl,
...
mod tests {
  ...
  fn comparison_should_work_with_multiple_invocations() {
    ...
    let value = Tensor::<TestBackend, 3, Bool>::from_primitive(comparison::<LowerEqual, f32, 3>(
      lhs.into_primitive(),
      rhs.into_primitive(),
    ));
  ...
  fn comparison_inplace_should_work_with_multiple_invocations() {
    ...
    let value =
      Tensor::<TestBackend, 3, Bool>::from_primitive(
        comparison_inplace::<LowerEqualInplace, f32, 3>(lhs.into_primitive(), rhs.into_primitive()),
      );
