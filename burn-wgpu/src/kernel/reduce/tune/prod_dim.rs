...
use crate::{
    ...
    kernel::{
        ...
        reduce::{init_reduce_output, prod_dim, prod_dim_shared_memory},
    },
...
/// Set of prod_dim implementations available for autotune
/// Autotune key is given by concatenating the closest upper power of 2 of
/// dim to reduce, and product of others
pub struct ProdDimAutotuneOperationSet<E: WgpuElement, const D: usize> {
    key: WgpuAutotuneKey,
    input: WgpuTensor<E, D>,
    output: WgpuTensor<E, D>,
    reduce_dim: usize,
}
impl<E: WgpuElement, const D: usize> ProdDimAutotuneOperationSet<E, D> {
    fn new(input: WgpuTensor<E, D>, output: WgpuTensor<E, D>, reduce_dim: usize) -> Self {
        Self {
            key: WgpuAutotuneKey::ProdDim(ReduceAutotuneKey::new(
                &input.shape,
                &input.strides,
                reduce_dim,
            )),
...
impl<E: WgpuElement + Element, const D: usize> AutotuneOperationSet<WgpuAutotuneKey>
    for ProdDimAutotuneOperationSet<E, D>
{
    ...
    fn autotunables(&self) -> Vec<Box<dyn AutotuneOperation>> {
        ...
        vec![
            Box::new(ProdDimAutotune::<E, D>::new(
                input.clone(),
                output.clone(),
                self.reduce_dim,
            )),
            Box::new(ProdDimSharedMemoryAutotune::<E, D>::new(
                input.clone(),
                output.clone(),
                self.reduce_dim,
            )),
    ...
    fn fastest(self: Box<Self>, fastest_index: usize) -> Box<dyn AutotuneOperation> {
        ...
        match fastest_index {
            0 => Box::new(ProdDimAutotune::<E, D>::new(
                self.input,
                self.output,
                self.reduce_dim,
            )),
            1 => Box::new(ProdDimSharedMemoryAutotune::<E, D>::new(
                self.input,
                self.output,
                self.reduce_dim,
            )),
...
/// Executes autotune on prod_dim operation
pub fn prod_dim_autotune<E: WgpuElement + Element, const D: usize>(
    input: WgpuTensor<E, D>,
    reduce_dim: usize,
) -> WgpuTensor<E, D> {
    ...
    let operation_set = Box::new(ProdDimAutotuneOperationSet::<E, D>::new(
        input,
        output.clone(),
        reduce_dim,
    ));
...
reduce_tune_ops!(ProdDimAutotune, prod_dim);

// Probably better on tensors large along reduce dim
reduce_tune_ops!(ProdDimSharedMemoryAutotune, prod_dim_shared_memory);
