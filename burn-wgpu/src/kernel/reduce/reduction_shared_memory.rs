...
mod tests {
    ...
    fn reduction_sum_dim_shared_memory_small() {
        let tensor = Tensor::<TestBackend, 1>::random_default([700], Distribution::Default);
        let tensor_ref = Tensor::<ReferenceBackend, 1>::from_data_default(tensor.to_data());
        let reduce_dim = 0;
    ...
    fn reduction_sum_dim_shared_memory_large() {
        let tensor = Tensor::<TestBackend, 3>::random_default([4, 1024, 50], Distribution::Default);
