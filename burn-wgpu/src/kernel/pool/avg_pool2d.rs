...
pub(crate) fn avg_pool2d_backward<E: WgpuElement>(
  ...
  let kernel: Box<dyn Kernel> = match count_include_pad {
    true => Box::new(StaticKernel::<
      KernelSettings<AvgPool2dBackward<true>, E, i32, WORKGROUP_DEFAULT, WORKGROUP_DEFAULT, 1>,
    >::new(workgroup)),
    false => Box::new(StaticKernel::<
      KernelSettings<AvgPool2dBackward<false>, E, i32, WORKGROUP_DEFAULT, WORKGROUP_DEFAULT, 1>,
    >::new(workgroup)),
  };
...
mod tests {
  ...
  fn avg_pool2d_backward_should_work_with_multiple_invocations() {
    ...
    let grad: Tensor<TestBackend, 4> = Tensor::from_primitive(TestBackend::avg_pool2d_backward(
