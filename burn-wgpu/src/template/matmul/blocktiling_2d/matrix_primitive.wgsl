...
fn main(
    ...
    for (var k = 0u; k < K; k += B_K) {
        ...
        for (var dot_index = 0u; dot_index < B_K; dot_index++) {
            ...
            let lhs_sm_position = thread_row + dot_index * B_M;
            let reg_M = vec4(
                shared_lhs[lhs_sm_position],
                shared_lhs[lhs_sm_position+1u],
                shared_lhs[lhs_sm_position+2u],
                shared_lhs[lhs_sm_position+3u]
            );
            // Load a subrow of values from rhs
            let rhs_sm_position = thread_col + dot_index * B_N;
            let reg_N = vec4(
                shared_rhs[rhs_sm_position],
                shared_rhs[rhs_sm_position+1u],
                shared_rhs[rhs_sm_position+2u],
                shared_rhs[rhs_sm_position+3u]
            );
            // Multiply subcolumn and subrow and store results
            // let vecmul = reg_M * reg_N;
            for (var res_idx_M = 0u; res_idx_M < T_M; res_idx_M++) {
                for (var res_idx_N = 0u; res_idx_N < T_N; res_idx_N++) {
                    results[res_idx_M * T_N + res_idx_N] += reg_M[res_idx_M] * reg_N[res_idx_N];
                }
            }
        }
