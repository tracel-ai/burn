...
// const MEM_SIZE = B_M * B_K / 4u;
// Memory size: B_M(or B_N) * B_K / 4 (vec size)
...
fn main(
    ...
    var results: array<{{ elem }}, 16u>; 
    // var results: mat4x4<{{ elem }}>; // todo try
    ...
    for (var k = 0u; k < K; k += B_K) {
        ...
        for (var j = 0u; j < 4u; j++) {
            ...
            let lhs_sm_position = current_row * (B_K/4u) + current_col; 
        ...
        for (var i = 0u; i < 4u; i++) {
            ...
            let rhs_sm_position = thread_col + current_row * (B_N/4u);
        ...
        for (var dot_index = 0u; dot_index < B_K; dot_index++) {
            ...
            let rhs_sm_position = thread_col + dot_index * (B_N/4u);
