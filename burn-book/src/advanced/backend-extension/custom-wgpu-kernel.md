...
The [example project](https://github.com/tracel-ai/burn/tree/main/examples/custom-wgpu-kernel)
...
This example supplies WGSL source to the WGPU runtime. It requires a compiler accepting WGSL;
a WGPU build selecting SPIR-V or MSL rejects this source. Registering `Cube` does not make a WGSL
kernel portable to CUDA, ROCm, or the CubeCL CPU runtime.
...
```rust,ignore
{{#include ../../../../examples/custom-wgpu-kernel/src/lib.rs}}
```
...
```wgsl
{{#include ../../../../examples/custom-wgpu-kernel/src/kernel.wgsl}}
```
...
```rust,ignore
{{#include ../../../../examples/custom-wgpu-kernel/src/forward.rs}}
```
...
```rust,ignore
{{#include ../../../../examples/custom-wgpu-kernel/src/backward.rs}}
```
