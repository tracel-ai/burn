...
When using the `tch` backend, Burn expects LibTorch to already be available on your system by
default. To enable automatic download, use `download-libtorch` (for example,
`cargo add burn --features "tch,download-libtorch"`).

For setup details, see [LibTorch Setup](./miscellaneous/libtorch-setup.md).
